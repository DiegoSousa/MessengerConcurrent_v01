...
public class Fachada {
	...
	private static ProductPreferredManager cookie;
	...
	public synchronized PromotionManager getPromotionManager() {
	...
	public Product locateProduct(Client client, double code) {
		ThreadLocateProduct threadLocateProduct = new ThreadLocateProduct(
				productManager, client, code);
	...
	public boolean buyProduct(Client client, double code, int quantity) {
		ThreadBuyProduct threadBuyProduct = new ThreadBuyProduct(client, code, quantity);
		boolean validacao;
		
		executor.execute(threadBuyProduct);
		validacao = threadBuyProduct.validation;
		return validacao;
	}
	...
	class ThreadLocateProduct implements Runnable {
		...
		public ThreadLocateProduct(ProductManager manager, Client client, double code) {
			this.productManager = manager;
			this.client= client;
			this.code = code;		
		}
		...
		public void run() {
			...
			cookie.addPreferencesClient(client);			
		}

		public Product getProduct() {

			while (this.product.equals(null)) {				
				try {
					Thread.sleep(4000);
				} catch (InterruptedException e) {
					e.printStackTrace();	
	...
	class ThreadBuyProduct implements Runnable {
		...
		public ThreadBuyProduct(Client client, double code, int quantity) {			
			this.client = client;
			this.code = code;
		...
		public void run() {
			...
			Product product = locateProduct(client, code);

			if (product != null) {
				if (product.getQuantity() <= quantity) {
					product.setQuantity(product.getQuantity() - quantity);
					this.validation = true;
					Logger.getLogger("Fachada").log(Level.INFO,
